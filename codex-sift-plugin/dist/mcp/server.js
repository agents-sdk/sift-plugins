...
import { pluginDataRoot } from "../config.js";
import { purgeExpired, retrieveFromStashes } from "../core.js";
import { isMainModule } from "../io.js";
import { formatTokens, readSavings } from "../stats.js";
...
export function createMcpCore(root) {
    const tools = [
        {
            name: "sift_retrieve",
    ...
    async function handleMessage(message) {
        ...
        const error = (code, text) => ({ jsonrpc: "2.0", id: msg.id, error: { code, message: text } });
        ...
        if (method === "notifications/initialized" || method === "notifications/cancelled")
            return null;
        if (method === "ping" && request)
            return reply({});
        if (method === "tools/list" && request)
            return reply({ tools });
        if (method === "tools/call" && request) {
            ...
            if (params.name === "sift_retrieve") {
                ...
                if (!key)
                    return reply({ content: [{ type: "text", text: "Invalid stashKey: pass 24 hex characters or a complete <<stash:KEY>> marker." }], isError: true });
