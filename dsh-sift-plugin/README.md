...
插件本身要求 Node.js `>=22.19`，以及 DeepSeek Harness `0.1.5-rc.2` 或兼容的 `0.1.x` 版本；通过 dsh SDK 或其他显式加载方式时，Node 23.11.1 可用。但当前 npm 发布的 dsh CLI 入口依赖 `import.meta.main`，因此标准 `dsh` 命令仍要求 Node.js `^22.19 || >=24.2`：Node 23.x 和 Node 24.0–24.1 会让 CLI 静默退出。先确认 `dsh --version` 可用；尚未安装时，先安装当前 developer preview CLI：
...
需要使用标准 dsh CLI 时，Volta 用户可运行 `volta install node@24`；nvm 用户可运行 `nvm install 24 && nvm use 24`，并确认得到的版本不低于 24.2。
