...
export function shouldSkipCompression(input, config) {
    if (config.excludedTools.includes(input.toolName))
        return "excluded";
    if (input.isError)
        return "error";
    if (input.truncated)
    ...
    if (!input.text)
        return "empty";
    if (utf8ByteLength(input.text) < config.minLength)
        return "too-short";
    if (containsValidStashMarker(input.text))
...
export function mergeSiftDetails(existing, outcome) {
