...
export type SiftPluginConfig = {
	enabled: boolean;
	minLength: number;
	excludedTools: string[];
};

export function parseEnabled(value: unknown, fallback = true): boolean {
...
export function parseMinLength(value: unknown): number {
...
export function parseMinLengthFromString(value: unknown): number {
	if (typeof value === "string" && value.trim() !== "") {
		return parseMinLength(Number(value));
	}
	if (typeof value === "number") return parseMinLength(value);
	return DEFAULT_MIN_LENGTH;
}

export function parseExcludedTools(value: unknown): string[] {
...
export function parseSiftConfig(
	input: {
		enabled?: unknown;
		minLength?: unknown;
